...
public class ScrapeIposFromIpo38Adapter implements ScrapeIposPort {
    ...
    private List<ScrapedIpoDto> scrapeIposFromPage(int pageNumber) {
        List<ScrapedIpoDto> result = new ArrayList<>();
        String url = "http://www.ipo38.co.kr/ipo/index.htm?o=&key=5&page=";
        String selector = "body > table:nth-child(5) > tbody > tr > td > table > tbody > tr > td:nth-child(1) >"
    ...
    private ScrapedIpoDto parseScrapedIpoDto(Element ipoRawData) throws IOException {
    ...
    private static List<String> getAdditionalInformation(String url) throws IOException {
    ...
    // '####.##:1' 형식에서 소수점 이전 부분을 int로 반환 / 정보가 없는 경우 0을 반환
